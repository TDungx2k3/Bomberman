...
public class Kondoria extends Enemy {
    public static final int KONDORIAPOINT = 250;

    public static final double KONDORIASPEED = 0.8;

    /**
     * Constructor Kondoria 3 parameters.
     *
     * @param x     - x
     * @param y     - y
     * @param board - board
    ...
    public Kondoria(int x, int y, Board board) {
        super(x, y, board, Sprite.kondoria_dead, KONDORIASPEED, KONDORIAPOINT);
            ...
            case 1:
                if (isMoving) {
                    _sprite = Sprite.movingSprite(Sprite.kondoria_right1,
                            Sprite.kondoria_right2, Sprite.kondoria_right3, _animate, 60);
                } else {
                    _sprite = Sprite.kondoria_left1;
                }
                break;
            ...
            case 3:
                if (isMoving) {
                    _sprite = Sprite.movingSprite(Sprite.kondoria_left1,
                            Sprite.kondoria_left2, Sprite.kondoria_left3, _animate, 60);
                } else {
                    _sprite = Sprite.kondoria_left1;
                }
                break;
    ...
    public void kill() {
        ...
        Message message = new Message("+" + KONDORIAPOINT, getXMessage(),
    ...
    public boolean canMoveRight() {
        Entity a = board.getEntity((this._x + 4) / Game.TILES_SIZE,
                (this._y) / Game.TILES_SIZE, this);
    ...
    public boolean canMoveUp() {
        Entity a = board.getEntity(this._x / Game.TILES_SIZE,
                (this._y - 4) / Game.TILES_SIZE, this);
        return a.collide(this);
    }
